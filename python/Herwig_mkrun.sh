...
cat Herwig_minkttable.txt | while read MINKT XS EVTS; do
	cmsDriver.py Herwig_QCD_Pt_$MINKT.py -s GEN --eventcontent RAWSIM --datatier GEN --conditions FrontierConditions_GlobalTag,STARTUP_V1::All -n $EVTS --no_exec
	echo "process.genParticles.abortOnUnknownPDGCode = False" >> Herwig_QCD_Pt_${MINKT}_py__GEN.py

	cmsDriver.py Herwig_QCD_Pt_$MINKT.py -s GEN,SIM,DIGI,L1,DIGI2RAW,HLT --eventcontent RAWSIM --datatier GEN-SIM-RAW --conditions FrontierConditions_GlobalTag,STARTUP_V1::All -n $EVTS --no_exec
