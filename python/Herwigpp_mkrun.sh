...
cat Herwigpp_minkttable.txt | while read MINKT XS EVTS; do
	cmsDriver.py Configuration/GenProduction/python/Herwigpp_QCD_Pt_$MINKT.py -s GEN --eventcontent RAWSIM --datatier GEN --conditions FrontierConditions_GlobalTag,STARTUP_V1::All -n $EVTS --no_exec
	echo "process.genParticles.abortOnUnknownPDGCode = False" >> Herwigpp_QCD_Pt_${MINKT}_py_GEN_STARTUP_V1.py

	cmsDriver.py Configuration/GenProduction/python/Herwigpp_QCD_Pt_$MINKT.py -s GEN,SIM,DIGI,L1,DIGI2RAW,HLT --eventcontent RAWSIM --datatier GEN-SIM-RAW --conditions FrontierConditions_GlobalTag,STARTUP_V1::All -n $EVTS --no_exec
