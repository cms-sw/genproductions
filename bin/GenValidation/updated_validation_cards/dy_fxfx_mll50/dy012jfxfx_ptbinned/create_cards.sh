...
do
	...
	if [[ "$bin_low" -eq "${Pt_array[5]}" ]];then
		...
		cp ./Zpt-0To50_cuts_template.f pt${bin_low}_inf/DYJetsToLL_012j_Zpt-${bin_low}Toinf_5f_NLO_FXFX_cuts.f
		sed -i '1s/0\~50/650\~inf/g' pt${bin_low}_inf/*cuts.f
		sed -i "420s/50/$bin_low/g" pt${bin_low}_inf/*cuts.f
		...
		cp ./Zpt-0To50_cuts_template.f pt${bin_low}_${bin_high}/DYJetsToLL_012j_Zpt-${bin_low}To${bin_high}_5f_NLO_FXFX_cuts.f
		if [[ "$bin_low" -ne "${Pt_array[0]}" ]];then
			sed -i "1s/0\~50/$bin_low\~$bin_high/g" pt${bin_low}_${bin_high}/*cuts.f
			sed -i "420s/50/$bin_high/g" pt${bin_low}_${bin_high}/*cuts.f
