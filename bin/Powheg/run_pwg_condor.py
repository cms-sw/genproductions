...
def runCommand(command, printIt = False, doIt = 1) :
    ...
    if doIt :
        try:
            data = check_output(command,shell=True, universal_newlines=True,stderr=STDOUT)
            status = 0
...
def runGetSource(parstage, xgrid, folderName, powInputName, process, noPdfCheck, tagName, svnRev) :
    ...
    powhegResProcesses = ["DIS", "b_bbar_4l", "HWJ_ew", "HW_ew", "HZJ_ew", "HZ_ew", "vbs-ssww-nloew", "WWJ", "ZZJ", "HJJ_ew", "LQ-s-chan", "gg4l", "Zgam", "ZgamJ", "VV_dec_ew", "WZJ", "Wc"]
