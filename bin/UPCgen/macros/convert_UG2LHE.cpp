...
void convert_UG2LHE(const std::string& inFileName, const double& beamE1, const double& beamE2)
{
  // Open input file
  std::ifstream inFile(inFileName);
  if (not inFile.is_open())
    throw std::logic_error("[convert_UG2LHE] Failed to open input file: "+inFileName);

  // Create output file
  const auto outFileName = inFileName.substr(0, inFileName.rfind(".")).substr(inFileName.rfind("/")+1) + ".lhe";
  std::ofstream outFile(outFileName);
  if (not outFile.is_open())
    throw std::logic_error("[convert_UG2LHE] Failed to open output file: "+outFileName);
  ...
  while (getline(inFile, line) &&
	...
	if (not (std::istringstream(line) >> label >> iEvt >> nVtx >> nPar) || label != "E")
      throw std::logic_error("[convert_UG2LHE] Failed to parse event line: "+line);
    ...
    for (size_t iPar=0; iPar < nPar; iPar++) {
      ...
      if (not getline(inFile, line) || not (std::istringstream(line) >> label >> parI >> momI >> pdgId >> px >> py >> pz >> en >> mass >> status) || label != "P" || parI != iPar+1 || momI >= parI)
        throw std::logic_error("[convert_UG2LHE] Failed to parse track line: "+line);
      ...
      if (momI > 0)
        ...
        momP += ROOT::Math::PxPyPzMVector(px, py, pz, mass);
    }

    // Add fake photons
    const double pz1 = momP.M2()/(2.*(momP.E() - momP.Pz())), pz2 = momP.Pz() - pz1;
    const double en1 = pz1 , en2 = sqrt(momP.P2() - 2*momP.Pz()*pz1 + pz1*pz1);
    std::ostringstream fp1, fp2;
    fp1 << "22 3 0 0 0 0 0.0 " << pz1 << " " << en1 << " 0.0 0.0 9.0" << std::endl;
    fp2 << "22 3 0 0 0 0 " << momP.Px() << " " << momP.Py() << " " <<  pz2 << " " << en2 << " 0.0 0.0 9.0" << std::endl;
    oLines[0] = fp1.str();
    oLines[1] = fp2.str();
...
int main(int argc, char *argv[])
{
  if (argc != 4) {
    ...
    std::cout << "Usage: ./convert_UG2LHE <INPUT_FILE> <BEAM_1_E> <BEAM_2_E>" << std::endl;
    return 1;
  }
  convert_UG2LHE(std::string(argv[1]), std::atof(argv[2]), std::atof(argv[3]));
  return 0;
};
