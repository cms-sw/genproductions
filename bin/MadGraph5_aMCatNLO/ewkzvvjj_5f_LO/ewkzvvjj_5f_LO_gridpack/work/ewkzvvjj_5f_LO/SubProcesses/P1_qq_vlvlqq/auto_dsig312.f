      DOUBLE PRECISION FUNCTION DSIG312(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(IPROC)=DX1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ b > ve ve~ u c~
      PD(IPROC)=SX1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ b > vm vm~ u c~
      PD(IPROC)=SX1*B2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG312 = PD(0)
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,312)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG312=DSIGN(PD(0)*CONV*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C     Generate events only if IMODE is 0.
      IF(IMODE.EQ.0.AND.DABS(DSIG312).GT.0D0)THEN
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG312*WGT,312)
      ENDIF
