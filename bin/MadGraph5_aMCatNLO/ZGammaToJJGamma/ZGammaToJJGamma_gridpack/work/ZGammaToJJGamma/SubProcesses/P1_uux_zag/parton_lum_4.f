...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! u u~ > z a d d~
        PD(IPROC) = U1*UX2
        IPROC=IPROC+1  ! u u~ > z a s s~
        PD(IPROC) = U1*UX2
        IPROC=IPROC+1  ! u u~ > z a b b~
        PD(IPROC) = U1*UX2
        IPROC=IPROC+1  ! c c~ > z a d d~
        PD(IPROC) = C1*CX2
        IPROC=IPROC+1  ! c c~ > z a s s~
        PD(IPROC) = C1*CX2
        IPROC=IPROC+1  ! c c~ > z a b b~
        PD(IPROC) = C1*CX2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
