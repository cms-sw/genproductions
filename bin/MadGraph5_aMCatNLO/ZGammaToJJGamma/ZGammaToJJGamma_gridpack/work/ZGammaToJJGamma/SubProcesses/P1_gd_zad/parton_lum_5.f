...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! u d > z a d u
        PD(IPROC) = U1*D2
        IPROC=IPROC+1  ! c d > z a d c
        PD(IPROC) = C1*D2
        IPROC=IPROC+1  ! u s > z a s u
        PD(IPROC) = U1*S2
        IPROC=IPROC+1  ! c s > z a s c
        PD(IPROC) = C1*S2
        IPROC=IPROC+1  ! u b > z a b u
        PD(IPROC) = U1*B2
        IPROC=IPROC+1  ! c b > z a b c
        PD(IPROC) = C1*B2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
