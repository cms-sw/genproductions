...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! d~ u~ > z a d~ u~
        PD(IPROC) = DX1*UX2
        IPROC=IPROC+1  ! d~ c~ > z a d~ c~
        PD(IPROC) = DX1*CX2
        IPROC=IPROC+1  ! s~ u~ > z a s~ u~
        PD(IPROC) = SX1*UX2
        IPROC=IPROC+1  ! s~ c~ > z a s~ c~
        PD(IPROC) = SX1*CX2
        IPROC=IPROC+1  ! b~ u~ > z a b~ u~
        PD(IPROC) = BX1*UX2
        IPROC=IPROC+1  ! b~ c~ > z a b~ c~
        PD(IPROC) = BX1*CX2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
