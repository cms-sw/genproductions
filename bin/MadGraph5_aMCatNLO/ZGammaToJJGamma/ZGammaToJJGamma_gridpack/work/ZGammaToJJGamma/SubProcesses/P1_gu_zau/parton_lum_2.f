...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! d~ u > z a u d~
        PD(IPROC) = DX1*U2
        IPROC=IPROC+1  ! s~ u > z a u s~
        PD(IPROC) = SX1*U2
        IPROC=IPROC+1  ! b~ u > z a u b~
        PD(IPROC) = BX1*U2
        IPROC=IPROC+1  ! d~ c > z a c d~
        PD(IPROC) = DX1*C2
        IPROC=IPROC+1  ! s~ c > z a c s~
        PD(IPROC) = SX1*C2
        IPROC=IPROC+1  ! b~ c > z a c b~
        PD(IPROC) = BX1*C2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
