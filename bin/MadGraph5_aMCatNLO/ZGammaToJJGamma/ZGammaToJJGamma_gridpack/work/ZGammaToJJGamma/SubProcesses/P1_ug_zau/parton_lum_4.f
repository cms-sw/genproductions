...
C         
      ...
      IF (IMIRROR.EQ.2) THEN
        ...
        PD(0) = 0D0
        IPROC = 0
        IPROC=IPROC+1  ! u d > z a u d
        PD(IPROC) = U1*D2
        IPROC=IPROC+1  ! u s > z a u s
        PD(IPROC) = U1*S2
        IPROC=IPROC+1  ! u b > z a u b
        PD(IPROC) = U1*B2
        IPROC=IPROC+1  ! c d > z a c d
        PD(IPROC) = C1*D2
        IPROC=IPROC+1  ! c s > z a c s
        PD(IPROC) = C1*S2
        IPROC=IPROC+1  ! c b > z a c b
        PD(IPROC) = C1*B2
      ELSE
        IF (ABS(LPP(1)) .GE. 1) THEN
          LP=SIGN(1,LPP(1))
