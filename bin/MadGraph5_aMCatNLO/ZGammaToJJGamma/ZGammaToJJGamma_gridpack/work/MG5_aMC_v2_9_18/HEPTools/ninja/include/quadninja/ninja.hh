...
#ifndef QUADNINJA_NINJA_HH
#define QUADNINJA_NINJA_HH

#define QUADNINJA_NINJA_HH_INSIDE
...
#include <quadninja/s_mat.hh>
#include <quadninja/num_defs.hh>
#include <quadninja/ninja_in.hh>
...
namespace quadninja {
  ...
  // quadninja::Amplitude is the main class of the ninja library.  The
  ...
  public:
    ...
    Amplitude & reset()
    {
      result[0] = result[1] = result[2] = quadninja::Complex();
      cut_constr = quadninja::Complex();
      return *this;
    }
  ...
  private:
      ...
      if (return_val & UNSTABLE_KINEMATICS) {
        if (Options::verb) {
          (*Options::out) << std::endl
                          << "quadninja::Amplitude is returning "
    ...
    // data
    quadninja::details::Array3D<Complex> result;
    SMatrix s_mat;
...
#undef QUADNINJA_NINJA_HH_INSIDE

#endif // QUADNINJA_NINJA_HH
