...
c PA: do not try to read the seed from iproc

c      open(unit=lun,file='./iproc.dat',status='old',err=15)
c         read(lun,*,err=14) iseed
c         close(lun)
c         return
c 14   close(lun)
c 15   open(unit=lun,file='../iproc.dat',status='old',err=25)
c         read(lun,*,err=24) iseed
c         close(lun)
c         return
c 24   close(lun)
c 25   iseed = 0
