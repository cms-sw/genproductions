...
c
      double precision function calc_eta(p)
c************************************************************************
...
c     Arguments
c
      double precision p(0:3), costh

c-----
c  Begin Code
c-----
      costh = p(3)/dsqrt(p(1)**2 + p(2)**2 + p(3)**2)
      if( dabs(costh) .lt. 0.999999 ) then
         calc_eta= 0.5*dlog((1.+costh)/(1.-costh))
      else
         if( p(3) .lt. 0 )then
            calc_eta = -100.
         else
      ...
      DOUBLE PRECISION  FUNCTION calc_phi(p)
c************************************************************************
...
c-----
c 
      if(dabs(p(1)) .gt. 1e-6) then
         if(p(1).gt.zero) then
            calc_phi=datan(p(2)/p(1))
         else if(p(1).lt.zero) then
            calc_phi=datan(p(2)/p(1))+pi
         endif
      else 
         if(p(2).ge.zero) then  !remind that p(1) <= 1e-6
            calc_phi=pi/2d0
         else if(p(2).lt.zero) then !remind that p(1) <= 1e-6
            calc_phi=-pi/2d0
         endif
      endif 
      if(calc_phi.lt.zero) calc_phi=calc_phi+2*pi
      return
      end
...
c Photon isolation
      ...
      double precision etalep,philep,etagam,phigam,delta_phi,delta_r,mindrla,mlnpmlnacut
      real*8 ptemp(0:3), ptemp2(0:3), ptemp3(0:3)

      real*8 PI
      parameter( PI = 3.14159265358979323846d0 )
C
      real*8   calc_eta,calc_phi
      external calc_eta,calc_phi
...
c are filled from the PDG codes (iPDG array) in this function.
     ...
     $     ,is_a_ph(nexternal),is_a_nu(nexternal)
...
c find the charged leptons (also used in the photon isolation cuts below)
     ...
     &    (ipdg(i).eq.-11 .or. ipdg(i).eq.-13 .or. ipdg(i).eq.-15)) then
         ...
         if(istatus(i).eq.1 .and.
     &    (ipdg(i).eq.12 .or. ipdg(i).eq.14 .or. ipdg(i).eq.16 .or. ipdg(i).eq.-12 .or. ipdg(i).eq.-14 .or. ipdg(i).eq.-16)) then
            is_a_nu(i)=.true.
         else
            is_a_nu(i)=.false.
         endif
...
C***************************************************************
      ...
      do j=0,3
         ptemp(j)=0  ! for lepton + neutrino
         ptemp2(j)=0 ! for the photon
         ptemp3(j)=0 ! for the lepton 
      enddo
      do i=nincoming+1,nexternal
          if( is_a_nu(i) ) then
c              if(debug) write (*,*) i,' -> neutrino '
              do j=0,3
                  ptemp2(j) = p(j,i)
              enddo
          elseif( is_a_lp(i) .or. is_a_lm(i) ) then
c              if(debug) write (*,*) i,' -> lepton '
              do j=0,3
                  ptemp(j) = p(j,i)
              enddo
          endif
...
c--   get the photon closest to a lepton
c--   here we copy copy how the root function works
      mindrla = -1.0
      mlnpmlnacut = -165.0
      etalep = calc_eta(ptemp)
      philep = calc_phi(ptemp)
      do i=nincoming+1,nexternal
         if(is_a_ph(i)) then            
            etagam = calc_eta(p(0,i))
            phigam = calc_phi(p(0,i))
            delta_phi = phigam - philep
            do while( delta_phi .ge. PI )  
               delta_phi = delta_phi - 2*PI
            enddo
            do while( delta_phi .lt. -PI )
               delta_phi = delta_phi + 2*PI
            enddo 
            delta_r = sqrt( (etagam - etalep)**2 + delta_phi**2 )
            if( delta_r .lt. mindrla .or. mindrla .eq. -1.0 ) then 
               mindrla = delta_r
c               if(debug) write (*,*) i,' -> photon '                  
               do j=0,3
                  ptemp3(j) = p(j,i)                  
               enddo         
            endif 
      ...
      enddo
      do j=0,3
         ptemp(j) = ptemp(j) + ptemp2(j)
         ptemp3(j) = ptemp3(j) + ptemp(j)
      enddo
      if( mlnpmlnacut .gt. 0.0 ) then
         if( dsqrt(dot(ptemp,ptemp)) + dsqrt(dot(ptemp3,ptemp3)) .le. mlnpmlnacut ) then
            passcuts_user = .false.
            return
         endif
      endif
      if( mlnpmlnacut .lt. 0.0 ) then
         if( dsqrt(dot(ptemp,ptemp)) + dsqrt(dot(ptemp3,ptemp3)) .gt. -mlnpmlnacut ) then
            passcuts_user = .false.
            return
         endif
      endif
