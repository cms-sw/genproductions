...
with open("ParticleWidths.csv") as csvfile:
	...
	for R in W:
		...
		Template_Replace("./"+N+"/"+N+"_customizecards.dat", "$XMASS$", '%e'%float(R[0]))
		Template_Replace("./"+N+"/"+N+"_customizecards.dat", "$XWIDTH$", '%e'%float(R[2]))
		Template_Replace("./"+N+"/"+N+"_customizecards.dat", "$AMASS$", '%e'%float(R[1]))
		Template_Replace("./"+N+"/"+N+"_customizecards.dat", "$AWIDTH$", '%e'%float(R[3]))
		Template_Replace("./"+N+"/"+N+"_run_card.dat", "$XQCUT$", '%e'%float(R[4]))
