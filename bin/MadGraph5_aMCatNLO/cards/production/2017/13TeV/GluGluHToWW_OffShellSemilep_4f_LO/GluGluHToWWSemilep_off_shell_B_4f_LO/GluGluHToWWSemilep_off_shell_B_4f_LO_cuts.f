c Follwing lines come from default cuts.f which is created if you run gridpack_generation.sh
c I add some user-defined lines(@~477, search USER DEFINED LINES] to avoid divergence of gg -> WW. The divergence occurs if transverse recoil by W boson is zero.
c For example, if there's no gluon emission and Wpt~0, it is a diverging case.
...
c     USER DEFINED
      ...
      double precision wpx,wpy,wpz,wwpx,wwpy,wwpz,wwee
      double precision wpsq,wwpsq,wpdotwwp ! p(w)**2 and p(ww)**2, p(w)*p(ww) in 3vector
C
C     PARAMETERS
C
      real*8 PI
...
c     To get lab momentum USER DEFINED
...
c     Also make sure there's no INF or NAN
      do i=1,nexternal
         ...
         enddo
      enddo
c Fill the arrays (momenta, status and PDG): ##USER DEFINED mimic setcuts.f
      
      do i=1,nexternal
         ipdg(i)=idup(i,1,1)
         if (ipdg(i).eq.-21) then 
            ipdg(i)=21
         endif
      enddo
c lab momenta USER DEFINED
c Boost the momenta p(0:3,nexternal) to the lab frame plab(0:3,nexternal)
      ybst_til_tolab=-cm_rap
...
c     [USER DEFINED LINES]
c     Let me add my cuts here my reference 
c     [https://github.com/cms-sw/genproductions/blob/master/bin/MadGraph5_aMCatNLO/cards/production/2017/13TeV/DYJets_ptbin_NLO/Zpt_0to50/DYJetsToLL_012j_Zpt-0To50_5f_NLO_FXFX_cuts.f]
c     loop->w,w.
c     require w pt wrt loop > 1GeV
      
c     To tag w leptonic's momenta
c      write (*,*), '--jhchoi--'
c      write (*,*), 'EBEAM(1)=',EBEAM(1)
c      write (*,*), 'EBEAM(2)=',EBEAM(2)
c      write (*,*), 'xbk(1)=',xbk(1)
c      write (*,*), 'xbk(2)=',xbk(2)
      
c      write (*,*), 'ybst_til_tolab=',ybst_til_tolab
c      write (*,*), 'cm_rap=',cm_rap
      do i=1,nincoming
c         write (*,*), 'p(0,i)=',p(0,i)
c         write (*,*), 'p(1,i)=',p(1,i)
c         write (*,*), 'p(2,i)=',p(2,i)
...
c         write (*,*), 'plab(0,i)=',plab(0,i)
c         write (*,*), 'plab(1,i)=',plab(1,i)
c         write (*,*), 'plab(2,i)=',plab(2,i)
c         write (*,*), 'plab(3,i)=',plab(3,i)
      enddo
c      write (*,*), ''
      wpx=0d0
      wpy=0d0
...
c      wwp3=0d0
c     Get wpx py
      do i=nincoming+1,nexternal-1 ! loop over all final particles
         do j=i+1,nexternal
            if((abs(ipdg(i)+ipdg(j)).eq.1) .and. (is_a_l(i) .or. is_a_l(j))) then ! if l,~v/ l~ v pair -> leptonic W+ or W-
c               write (*,*), '--wlep tagged--'
c               write (*,*), 'ipdg(i)=',ipdg(i)
c               write (*,*), 'ipdg(j)=',ipdg(j)

c               wpx=p(1,i)+p(1,j)
c               wpy=p(2,i)+p(2,j)
c               wpz=p(3,i)+p(3,j)

               wpx=plab(1,i)+plab(1,j)
               wpy=plab(2,i)+plab(2,j)
               wpz=plab(3,i)+plab(3,j)
               exit
            endif
         enddo
      enddo
...
c     Get ww momentum
      do i=nincoming+1,nexternal ! loop over all final particles
         write (*,*), ipdg(i)
         if(ipdg(i).ne.21) then ! if not gluon -> fianl state not gluon isr -> from ww
c            write (*,*), 'not gluon'
c            write (*,*),'ee:',p(0,i)
c            write (*,*),'px:',p(1,i)
c            write (*,*),'py:',p(2,i)
...
c            write (*,*),'eelab:',plab(0,i)
c            write (*,*),'pxlab:',plab(1,i)
c            write (*,*),'pylab:',plab(2,i)
c            write (*,*),'pzlab:',plab(3,i)
...
c      write (*,*), 'wwee=',wwee
      ...
      wwpsq=wwpx*wwpx+wwpy*wwpy+wwpz*wwpz
      wpsq=wpx*wpx+wpy*wpy+wpz*wpz
      wpdotwwp = wwpx*wpx+wwpy*wpy+wwpz*wpz
c     Pt(w) wrt P(ww) < 1gev
c     <=>(in 3vector)  [p(w)p(ww)]**2 - [p(w) dot p(ww)]**2 < p(ww)**2 [all p's in GeV]
c     <=>(in 3vector) p(w)p(ww)sqrt(1-cos**2) < p(ww)
c     <=>(in 3vector) p(w) * sin < 1
c     Here, sin = sin[theta between p(w) and p(ww)]
c      write (*,*), 'wwpsq',wwpsq
c      write (*,*), 'wpsq',wpsq
c      write (*,*), 'wpdotwwp',wpdotwwp
c      write (*,*), 'LHS',wpsq*wwpsq-wpdotwwp*wpdotwwp
c      write (*,*), 'RHS',wwpsq
c      write (*,*), 'pt wrt pww', dsqrt((wpsq*wwpsq-wpdotwwp*wpdotwwp)/wwpsq)
c      write (*,*), "pt", dsqrt(wpx*wpx+wpy*wpy)
      if((wpsq*wwpsq-wpdotwwp*wpdotwwp).lt.wwpsq)then
         passcuts=.false.
         return
      endif
c     
...
c     USER DEFINED ->is it copy from nlo template
