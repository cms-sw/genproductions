...
REFERENCE_DIR = "Top32_1000_gaChannel"
...
def formatName(mass):
    return "Top32_%d_gaChannel" % (mass)
...
for mass in MASSES:
    ...
    with open("{0}/{0}_proc_card.dat".format(sampleName), "w") as f:
        ...
        f.write("set loop_color_flows False\n")
        f.write("set gauge unitary\n")
        f.write("set complex_mass_scheme False\n")
        f.write("set max_npoint_for_channel 0\n")
        ...
        f.write("generate p p > tstar tstar~ , ( tstar > t g , ( t > b w+ , w+ > wdec wdec ) ) , ( tstar~ > t~ a , ( t~ > b~ w- , w- > wdec wdec ) )\n")
        f.write("output Top32_%d_gaChannel -nojpeg\n" % mass)
