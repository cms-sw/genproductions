...
until [ $index -eq ${#masses[@]} ]; do
    ...
    sed -i "s/0.537/${widths[${index}]}/g" LQToBMu_madgraph_LO_single-M${masses[$index]}/LQToBMu_madgraph_LO_single-M${masses[$index]}_param_card.dat
    sed -i "s/0.537/${widths[${index}]}/g" LQToBMu_madgraph_LO_single-M${masses[$index]}/LQToBMu_madgraph_LO_single-M${masses[$index]}_customizecards.dat
    let index=${index}+1
