#!/bin/bash
code_dir=$PWD
nevents=(1000000 1000000 1000000 1000000 1000000 1000000 500000 500000 500000 100000 100000 100000 100000 100000 100000 100000 100000 100000)

declare -i i=0
for mass in 5 10 20 30 50 70 100 130 150 200 250 300 350 400 500 600 800 1000
do
    cd $code_dir
    echo $mass;
    dir1=$code_dir"/RHTauNeutrino_M-"$mass"_13TeV-2017-2018_madgraph_MLM"
    mkdir $dir1
    cd $dir1
	...
	#done
	customize_card=RHTauNeutrino_M-$mass"_13TeV-2017-2018_madgraph_MLM_customizecards.dat"
	extramodels=RHTauNeutrino_M-$mass"_13TeV-2017-2018_madgraph_MLM_extramodels.dat"
	proc_card=RHTauNeutrino_M-$mass"_13TeV-2017-2018_madgraph_MLM_proc_card.dat"
	run_card=RHTauNeutrino_M-$mass"_13TeV-2017-2018_madgraph_MLM_run_card.dat"	
	touch $customize_card
 	touch $extramodels
	touch $proc_card
	touch $run_card	
	echo "writing customize card for mass " $mass " in " $mode " mode "
	echo "set param_card mass 66 "$mass".0" >> $customize_card
	echo "set param_card decay 66 AUTO" >> $customize_card
	echo "set param_card bmlblock thm 0.000000e+00" >> $customize_card
	echo "set param_card bmlblock the 0.000000e+00" >> $customize_card
	echo "set param_card bmlblock tht 1.000000e-02" >> $customize_card

	echo "writing extramodels card for mass " $mass " in " $mode " mode "

	echo "SM_RHN_UFO.tgz" >> $extramodels
	
	echo "writing proc_card for mass " $mass " in " $mode " mode "
	echo "import model sm" >> $proc_card
	echo "define p = g u c d s u~ c~ d~ s~" >> $proc_card
	echo "define j = g u c d s u~ c~ d~ s~" >> $proc_card
	echo "define l+ = e+ mu+" >> $proc_card
	echo "define l- = e- mu-" >> $proc_card
	echo "define vl = ve vm vt" >> $proc_card
	echo "define vl~ = ve~ vm~ vt~" >> $proc_card
	echo "import model SM_RHN_UFO" >> $proc_card
	echo "define lep = e+ e- mu+ mu- ta+ ta-" >> $proc_card
	...
	echo "output RHTauNeutrino_M-"$mass"_13TeV-2017-2018_madgraph_MLM -nojpeg" >> $proc_card
	echo "" >> $proc_card
	...
	echo "#*********************************************************************" >> $run_card
	echo "  "${nevents[i]}" = nevents ! Number of unweighted events requested " >> $run_card
...
#	echo "     230000    = lhaid     ! if pdlabel=lhapdf, this is the lhapdf number" >> $run_card
	...
	echo "#*********************************************************************" >> $run_card
	echo " 0.0 = time_of_flight ! threshold below which info is not written" >> $run_card
	echo "#*********************************************************************" >> $run_card
