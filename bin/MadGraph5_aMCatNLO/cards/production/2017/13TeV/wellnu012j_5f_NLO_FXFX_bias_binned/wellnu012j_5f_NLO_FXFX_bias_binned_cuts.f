...
c
      ...
      include 'nexternal.inc'
      include 'cuts.inc'
      double precision bias_wgt,p(0:3,nexternal),H_T,ptw,ptw_wgt,njets_wgt
      integer ipdg(nexternal),i,j,njets
      integer len_x1,len_x2,njet
      parameter (len_x1=30,len_x2=40)
      double precision, dimension(len_x1) ::  wt_vals_1
      double precision, dimension(len_x2) ::  wt_vals_2

      wt_vals_1 = (/0.44865,0.44865,0.44865,0.44865,0.712498,
     &1.23459,2.00486,3.43926,5.4388,8.70784,13.3225,20.0741,
     &29.0328,43.8323,55.2566,75.4548,104.081,138.413,188.011,
     &269.649,365.227,458.886,540.3,703.483,841.816,1471.43,
     &1312.25,1914.81,2355.65,2452.7/)
      wt_vals_2 = (/5.76768,5.76768,5.76768,5.83989,6.73349,
     &7.52133,8.96808,11.0716,14.2292,18.6614,24.8046,32.5577,
     &43.0167,55.1656,71.8655,91.4916,114.517,142.86,183.198,
     &232.1,278.587,326.062,400.994,501.396,601.955,745.739,
     &896.077,1061.11,1178.66,1391.79,1697.03,1972.75,2365.08,
     &2694.75,3124.82,3705.91,4250.14,4598.67,5064.54,6043.82/)
...
!       subtract real emission from jet counting
      ...
      if (njets.eq.0) then
        ...
        if (ptw.lt.300) then
          bias_wgt = wt_vals_1(floor(ptw/10d0)+1)
        else
          bias_wgt = wt_vals_1(30)
        endif
      else if (njets.eq.2) then
        if (ptw.lt.400) then
          bias_wgt = wt_vals_2(floor(ptw/10d0)+1)
        else
