...
C
      ...
      DOUBLE PRECISION PTMINSAVE, RHOPYTHIA, invm2_04
      EXTERNAL RHOPYTHIA
C
...
c mll bins
      ...
      enddo
      if (sqrt(sump(0)**2 - sump(1)**2 - sump(2)**2 - sump(3)**2).lt.750) then 
         passcuts=.false.
         return
      endif
      if (sqrt(sump(0)**2 - sump(1)**2 - sump(2)**2 - sump(3)**2).gt.1000) then 
         passcuts=.false.
         return
      endif
