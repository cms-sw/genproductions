...
c
      ...
      include 'nexternal.inc'
      include 'cuts.inc'
      double precision bias_wgt,p(0:3,nexternal),H_T,ptz,ptz_wgt,njets_wgt
      integer ipdg(nexternal),i,j,njets
      integer len_x1,len_x2,njet
      parameter (len_x1=35,len_x2=40)
      double precision, dimension(len_x1) ::  wt_vals_1
      double precision, dimension(len_x2) ::  wt_vals_2

      wt_vals_1 = (/0.465257,0.465257,0.465257,0.465257,0.719037,
     &1.14739,1.95044,3.20789,5.13718,7.66451,11.7727,16.8712,24.3524,
     &37.9325,49.1546,68.8715,92.2811,128.882,155.782,226.179,290.897,
     &412.004,477.387,571.178,756.516,905.214,1175.85,1257.92,
     &1532.0,1903.63,2324.71,2946.18,3788.65,4276.16,5741.26/)
      wt_vals_2 = (/5.08477,5.08477,5.08477,5.27331,5.63957,6.36419,
     &7.53864,8.87146,11.246,14.413,18.5502,24.5158,31.5455,40.7312,
     &51.7811,64.42,80.4664,104.554,126.924,158.292,192.461,239.424,
     &287.955,358.154,399.371,479.877,586.994,707.606,832.061,
     &962.437,1104.04,1360.27,1543.19,
     &1748.12,2111.33,2481.56,2804.63,3138.91,3421.69,3928.96/)
...
!       subtract real emission from jet counting
      ...
      if (njets.eq.0) then
        bias_wgt=0.4207
      else if (njets.eq.1) then
        if (ptz.lt.350) then
          bias_wgt = wt_vals_1(floor(ptz/10d0)+1)
        else
          bias_wgt = wt_vals_1(35)
        endif
      else if (njets.eq.2) then
        if (ptz.lt.400) then
          bias_wgt = wt_vals_2(floor(ptz/10d0)+1)
        else
          bias_wgt = wt_vals_2(40)
        endif
      endif
