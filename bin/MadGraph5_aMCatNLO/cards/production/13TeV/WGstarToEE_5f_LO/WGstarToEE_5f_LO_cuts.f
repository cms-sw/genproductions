c *This file contains minimum cut(M<4) on at least one SF OS dilepton 
c pair: line 71-77, 143-146, 160-166
c
...
c are filled from the PDG codes (iPDG array) in this function.
     ...
     $     ,is_a_ph(nexternal)
C EXTRA counter to keep track of number of SF OS leptons within mll range
      integer nSFOSoutRange
      integer nSFOS

      passcuts_user=.true. ! event is okay; otherwise it is changed
      nSFOSoutRange=0 ! Start out having 0 such
      nSFOS=0 ! count total SF OS lepton pairs
...
c DeltaR and invariant mass cuts
               ...
               do j=nincoming+1,nexternal
                  if (is_a_lm(j)) then
                     ...
                     if (ipdg(i).eq.-ipdg(j)) then
                        ...
                        if (invm2_04(p(0,i),p(0,j),1d0).gt.4**2)
     $                       then
                           nSFOSoutRange=nSFOSoutRange+1
                        endif
     ...
     $                          then
      ...
      enddo
c 
c Now check the extra cuts
c
      if (nSFOS.eq.nSFOSoutRange) then
         passcuts_user=.false.
         return
      endif
c
