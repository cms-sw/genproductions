...
C     file, others are in ./Source/kin_functions.f
      REAL*8 R2_04,invm2_04,invm3_04,pt_04,eta_04,pt,eta
      external R2_04,invm2_04,invm3_04,pt_04,eta_04,pt,eta
c local integers
      integer i,j
...
c
      ...
      do i=1,nexternal
         if (istatus(i).eq.1 .and. (abs(ipdg(i)).eq.24)) then
            if (pt_04(p(0,i)).lt.150.) then
               passcuts_user=.false.
               return
            endif
         endif
     ...
     &              abs(ipdg(k)).eq.24) then
                  if (invm3_04(p(0,i),p(0,j),p(0,k),1d0).mWVCUTLOW.**2) then
                     passcuts_user=.false.
                     return
                  endif
                  if (invm3_04(p(0,i),p(0,j),p(0,k),1d0).mWVCUTHIGH.**2) then
                     passcuts_user=.false.
                     return
                  endif
               endif
...
c-----
      ...
      DOUBLE PRECISION FUNCTION invm3_04(P1,P2,P3,dsign)
c************************************************************************
...
c     Arguments
c
      double precision p1(0:4),p2(0:4),p3(0:4),dsign
c
c     Local
c
      integer i
...
c-----
      ...
      do i=0,3
         ptot(i)=p1(i)+dsign*p2(i)+dsign*p3(i)
      enddo
      invm3_04 = dot(ptot,ptot)
      RETURN
      END
