...
generate p p > tp tp~, ( tp > g t, ( t > b w+, w+ > wdecay wdecay ) ), ( tp~ > a t~, ( t~ > b~ w-, w- > wdecay wdecay ) )
add process p p > tp tp~, ( tp > a t, ( t > b w+, w+ > wdecay wdecay ) ), ( tp~ > g t~, ( t~ > b~ w-, w- > wdecay wdecay ) )

output TpTp_M%i_ttga_channel -nojpeg
"""

modelCardData="""top3decays_ufo.tgz
"""

customCardData="""#put card customizations here (change top and higgs mass for example)
...
for m in MassList:
    if not os.path.exists(f"TpTp_M{m}_ttga_channel"):
        os.mkdir(f"TpTp_M{m}_ttga_channel")

    with open (f"TpTp_M{m}_ttga_channel/TpTp_M{m}_ttga_channel_customizecards.dat",'w') as _file:
        _file.write(customCardData%m)

    with open (f"TpTp_M{m}_ttga_channel/TpTp_M{m}_ttga_channel_extramodels.dat",'w') as _file:
        _file.write(modelCardData)

    with open (f"TpTp_M{m}_ttga_channel/TpTp_M{m}_ttga_channel_run_card.dat",'w') as _file:
        _file.write(runCardData)

    with open (f"TpTp_M{m}_ttga_channel/TpTp_M{m}_ttga_channel_proc_card.dat",'w') as _file:
        _file.write(procCardData%m)
