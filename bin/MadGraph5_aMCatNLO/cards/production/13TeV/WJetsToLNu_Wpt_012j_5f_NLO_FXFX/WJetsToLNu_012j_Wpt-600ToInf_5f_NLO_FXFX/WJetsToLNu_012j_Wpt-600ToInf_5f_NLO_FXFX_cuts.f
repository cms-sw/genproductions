...
c local integers
      integer i,j
c      integer i,j,iplusone,iminusone
...
c
     ...
     &        abs(ipdg(i)).eq.16).and.(ipdg(j).eq.-sign(abs(ipdg(i))-1,ipdg(i)))).or.
     &        ((abs(ipdg(i)).eq.11.or.abs(ipdg(i)).eq.13.or.
     &        abs(ipdg(i)).eq.15).and.(ipdg(j).eq.-sign(abs(ipdg(i))+1,ipdg(i))))) then
              if (ptZ(p(0,i),p(0,j)).lt.600) then
                  passcuts_user=.false.
                  return
              endif
            endif
