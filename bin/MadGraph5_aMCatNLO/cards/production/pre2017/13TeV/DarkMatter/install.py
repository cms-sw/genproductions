...
for med    in args1.medrange:
    for dm in args1.dmrange:
        ...
        if not args1.resubmit:
            ...
            #job_file.write('cd - \n')
            #job_file.write('cd -  \n')
            job_file.write('scramv1 project CMSSW CMSSW_7_1_15_patch1 \n')
            job_file.write('cd CMSSW_7_1_15_patch1/src \n')
            ...
            if len(cust) > 0:
                job_file.write('cp %s %s_%s_%s/                   \n' % (cust[0],args1.name,med,dm))
            ...
            job_file.write('cp    %s/runcmsgrid.sh    .      \n'  % (basedir))
        ...
        if os.path.isfile('%s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (basedir,args1.name,args1.name,med,dm)):
            os.system('bsub -q  %s -R "rusage[mem=12000]" %s/%s_MG5_aMC_v2_2_2/MG_%s_%s_%s/integrate.sh' % (args1.queue,basedir,args1.name,args1.name,med,dm))
