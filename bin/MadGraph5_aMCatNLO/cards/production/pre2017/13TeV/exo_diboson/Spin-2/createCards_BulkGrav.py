...
for H_decay in H_decays :
    for mass in mass_points :
        ...
        directoryName = str('./BulkGraviton_hh_narrow/BulkGraviton_hh'+H_decay+'_narrow_M'+mass)
        print directoryName
        if generate==1 :
            os.makedirs( directoryName )
            os.chdir( directoryName )

            run_card = open( 'BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_run_card.dat', 'w' )
            run_card.write( run_content )
            run_card.close()

            extra_card = open( 'BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_extramodels.dat', 'w' )
            extra_card.write( extra_content )
            extra_card.close()

            customize_card = open( 'BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_customizecards.dat', 'w' )
            customize_card.write( custom_content )
            customize_card.close()

            for line in fileinput.input('BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_customizecards.dat', inplace=True): 
            ...
            proc_card = open( 'BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_proc_card.dat', 'w' )            
            proc_card.write( proc_content )
            proc_card.close()
            
            for line in fileinput.input('BulkGraviton_hh'+H_decay+'_narrow_M'+mass+'_proc_card.dat', inplace=True): 
                print line.rstrip().replace('BulkGraviton_hh_hdecay_narrow_Mmass', 'BulkGraviton_hh'+H_decay+'_narrow_M'+mass),'\n' 
