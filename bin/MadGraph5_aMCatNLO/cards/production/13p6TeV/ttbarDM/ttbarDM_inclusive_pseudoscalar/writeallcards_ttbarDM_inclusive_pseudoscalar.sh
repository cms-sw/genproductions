...
for MCHI in 1 20 30 40 45 49 51 55; do
    if ((${MCHI}==1))
	   ...
	   for MPHI in 50 100 150 200 250 300 350 400 450 500; do
	       ...
	       sed "s/.*output.*/output ttbarDM__inclusive__DMsimp_LO_ps_spin0__mchi_${MCHI}_mphi_${MPHI}_gSM_1_gDM_1_6800GeV -nojpeg/" ${TEMP}/${PROC}_proc_card.dat > "${JOBS}/${MODEL}/${MODEL}_proc_card.dat"
	   done
    else
	MPHI=100
        MODEL=${PROC}"__mchi_"${MCHI}"_mphi_"${MPHI}"_gSM_1_gDM_1_6800GeV"
