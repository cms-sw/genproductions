...
template_dir = "WprimeToTauNu_M-6000_kR-0.01_LO_NNPDF31nnlo/"
...
#### Loop through all couplings ####
    for coupling in kR: 
        ...
        with open(directory+"/"+directory+"_proc_card.dat", "w") as new_proc_file:  # Create the output proc card file
            with open(template_dir+'WprimeToTauNu_M-6000_kR-0.01_LO_NNPDF31nnlo_proc_card.dat', 'r') as f_proc: # Read in the template file
                for line in f_proc:
...
### Copy the run card an the extramodels and adjust the filename as they do not need any changes                        
        shutil.copy(template_dir + 'WprimeToTauNu_M-6000_kR-0.01_LO_NNPDF31nnlo_run_card.dat',directory + '/' + directory + '_run_card.dat')
        shutil.copy(template_dir + 'WprimeToTauNu_M-6000_kR-0.01_LO_NNPDF31nnlo_extramodels.dat',directory + '/' + directory + '_extramodels.dat')
