...
template_dir = "WprimeToENu_M-1000_kR-0p01_LO_NNPDF31nnlo/"
#### Loop through all masses ####
for mass in masses:
#### Loop through all couplings ####
    for coupling in kR: 
        print(WprimeWidth(mass,coupling))
        directory = ("WprimeToENu_M-"+str(mass)+"_kR-"+str(coupling)+"_LO_NNPDF31nnlo").replace(".","p") # Set the output name
        ...
        with open(directory+"/"+directory+"_customizecards.dat", "w") as new_cust_file: # Create the output customizecards file
            with open(template_dir+ 'WprimeToENu_M-1000_kR-0p01_LO_NNPDF31nnlo_customizecards.dat', 'r') as f_cust: # Read in the template file 
        ...
        with open(directory+"/"+directory+"_proc_card.dat", "w") as new_proc_file:  # Create the output proc card file
            with open(template_dir+'WprimeToENu_M-1000_kR-0p01_LO_NNPDF31nnlo_proc_card.dat', 'r') as f_proc: # Read in the template file
                for line in f_proc:
...
### Copy the run card an the extramodels and adjust the filename as they do not need any changes                        
        shutil.copy(template_dir + 'WprimeToENu_M-1000_kR-0p01_LO_NNPDF31nnlo_run_card.dat',directory + '/' + directory + '_run_card.dat')
        shutil.copy(template_dir + 'WprimeToENu_M-1000_kR-0p01_LO_NNPDF31nnlo_extramodels.dat',directory + '/' + directory + '_extramodels.dat')
