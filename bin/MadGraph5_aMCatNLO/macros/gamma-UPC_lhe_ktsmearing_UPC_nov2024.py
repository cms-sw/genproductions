...
def generate_Q2_epa_ion(ibeam,x,Q2max,RA,aA,wA):
    ...
    while True:
        ...
        n_rand+=1
        if n_rand == int(1e6):
            print("\nWARNING: It's maybe impossible to find a correct answer")
            print("logQ2oQ02max,logQ2oQ02min,w =" ,logQ2oQ02max,logQ2oQ02min,w)
        if n_rand == int(1e7):
...
for i,file in enumerate(files):
    N_event=0
    stream=open(file,'r')
    print("INFO: Counting the number of events in file")
    for line in stream:
        if "<event>" in line or "<event " in line: N_event +=1
    print("INFO: Number of events in file = %d"%N_event)
    stream.close()
    stream=open(file,'r')
    headQ=True
    initQ=False
    ...
    procid=None
    proc_dict={}
    print("INFO: Start processing the file")
    for line in stream:
        sline=line.replace('\n','')
        if "<init>" in line or "<init " in line:
            ...
            if iinit == 1:
                if i == 0:
                    ...
                    if abs(PID_beam1) != 2212 or abs(PID_beam2) != 2212:
                        print("Not a proton-proton collider")
                        raise ValueError
                    ...
                    if not sline == firstinit:
                        print("the beam information of the LHE files is not identical")
                        raise Exception
            elif iinit >= 2:
                ...
                print("should not reach here. Do not understand the <init> block")
                raise Exception
            ...
            if ievent == 1:
                ...
                for procid,new_procid in proc_dict.items():
                    ...
                    break
                if not found:
                    print("do not find the correct proc id !")
                    raise Exception
                ...
                if ievent == n_particles+1:
                    ...
                    Ecm=2*numpy.sqrt(E_beam1*E_beam2)
                    pext_new = None
                    Q1=0
                    Q2=0
                    while pext_new == None:
...
stream.write(text)
stream.close()
print("\nINFO: The final produced lhe file is %s"%outfile)
if nan_count > 0:
    print("INFO: The ratio of nan is %f"%(nan_count/nevent),end=' ')
    print("for %d events"%nevent)
if nan_count/nevent > 0.01:
    print("WARNING: The ratio of nan is too large, please check the input lhe files")
