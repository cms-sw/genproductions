...
c                        *
c     ------------------*
c       no resonance     * ________ missing    p2
...
c
     ...
     & inv_matching_type_part
      ...
      momenta(0,p2)=-1d0
      jac_factor=1d0
      smin=(Etot+dsqrt(miss_px**2+miss_py**2))**2
      if(smin.gt.s) then
      jac=-1d0
      return
      endif
...
C     IF THERE IS NO S CHANNEL POLE USE BELOW:
      ...
      TAUMAX = 1D0
      jac_loc = 1d0
      call  generate_flat(x,1, TAUMIN, TAUMAX, TAU, jac_loc)
C     FROM HERE ON SAME WITH OR WITHOUT POLE
      ETAMIN = .5d0*LOG(TAU)
      ETAMAX = -ETAMIN
      call  generate_flat(x,2, ETAMIN, ETAMAX, ETA, jac_loc)
...
c      fill intermediate momenta
      do nu=0,3
       momenta(nu,r1)=momenta(nu,p1)+momenta(nu,p3)
       momenta(nu,r2)=momenta(nu,p2)+momenta(nu,p4)
      enddo
