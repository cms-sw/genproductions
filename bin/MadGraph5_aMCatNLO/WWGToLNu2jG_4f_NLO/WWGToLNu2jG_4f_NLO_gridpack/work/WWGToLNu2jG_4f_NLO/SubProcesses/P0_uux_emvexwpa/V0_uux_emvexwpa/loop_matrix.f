...
C      DIFFERENT EVALUATION METHODS IN ORDER TO ASSESS STABILITY.
     ...
     $ /NSQSOXNLG*.FALSE./
      ...
      COMMON/RUNTIME_OPTIONS/AUTOMATIC_CACHE_CLEARING
...
C     
      ...
      DATA N_DP_EVAL/1/
      DATA N_QP_EVAL/1/
      COMMON/N_EVALS/N_DP_EVAL,N_QP_EVAL
...
C         SET THE HELICITY FILTER
          IF(.NOT.FOUNDHELFILTER) THEN
            ...
            IF(ISZERO(DABS(HELSAVED(1,HELPICKED))+DABS(HELSAVED(2
...
C                 Be looser for helicity check, bring a factor 100
                  DUMMY=ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1,H),REF
     $             ,.FALSE.)
                  IF(DUMMY.NE.0) THEN
                    IF(NTRY.EQ.1) THEN
...
C         SET THE LOOP FILTER
          IF(.NOT.FOUNDLOOPFILTER.AND.USELOOPFILTER) THEN
            DO I=1,NLOOPGROUPS
              DO J=1,NSQUAREDSO
                IF(.NOT.ISZERO(ABS(LOOPRES(1,J,I))+ABS(LOOPRES(2,J,I))
     $           +ABS(LOOPRES(3,J,I)),(REF*1.0D-4),I,J)) THEN
                  IF(NTRY.EQ.1) THEN
                    GOODAMP(J,I)=.TRUE.
...
C         DOUBLE CHECK THE HELICITY FILTER
          IF (GOODHEL(HELPICKED).EQ.-HELOFFSET) THEN
            IF (.NOT.ISZERO(DABS(HELSAVED(1,HELPICKED))
 ...
 348          CONTINUE
          ...
          IF (GOODHEL(HELPICKED).LT.-HELOFFSET.AND.NTRY.NE.0) THEN
            IF(ISSAME(HELSAVED(1,HELPICKED),HELSAVED(1
...
C     --=========================================--
      ...
      LOGICAL FUNCTION IS_HEL_SELECTED(HELID)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
      ...
      LOGICAL FUNCTION ISZERO(TOTEST, REFERENCE_VALUE, LOOP, SOINDEX)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     ----------
     ...
     $   //' zero.'
      ...
      IF(LOOP.NE.-1) THEN
        IF((.NOT.ISZERO).AND.(.NOT.S(SOINDEX,LOOP))) THEN
...
C      inconsistency, then we only allow for weight one comparisons.
      ...
      DO I=1,N_WGT_TO_TRY
        DO J=1,3
          IF (ISZERO(ABS(RESB(J)),REF,-1,-1)) THEN
            IF(.NOT.ISZERO(ABS(RESB(J))+ABS(RESA(J)),REF,-1,-1)) THEN
              GOTO 1231
            ENDIF
C           Be looser for helicity comparison, so bring a factor 100
          ELSEIF(.NOT.ISZERO(ABS((RESA(J)/RESB(J))-DBLE(WGT_TO_TRY(I)))
     $     ,1.0D0,-1,-1)) THEN
            GOTO 1231
          ENDIF
        ENDDO
...
C     
      ...
      SUBROUTINE GET_ANSWER_DIMENSION(ANS_DIM)
C     
...
C     
      ...
      INTEGER FUNCTION ML5SOINDEX_FOR_SQUARED_ORDERS(ORDERS)
C     
...
C     
      DO I=1,NSQSO
        ...
        ENDDO
        ML5SOINDEX_FOR_SQUARED_ORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO
     ...
     $ //' ML5SOINDEX_FOR_SQUARED_ORDERS'
      ...
      INTEGER FUNCTION ML5SOINDEX_FOR_BORN_AMP(AMPID)
C     
...
C     
      ...
      DATA (BORNAMPORDERS(I),I=  1,  5) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I=  6, 10) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I= 11, 15) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I= 16, 20) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I= 21, 25) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I= 26, 30) /    1,    1,    1,    1,    1/
      DATA (BORNAMPORDERS(I),I= 31, 32) /    1,    1/
C     -----------
C     BEGIN CODE
C     -----------
     ...
     $   //' maximum ',NBORNAMPS
      ENDIF
      ML5SOINDEX_FOR_BORN_AMP = BORNAMPORDERS(AMPID)

      END

      INTEGER FUNCTION ML5SOINDEX_FOR_LOOP_AMP(AMPID)
C     
...
C     
      ...
      DATA (LOOPAMPORDERS(I),I=161,162) /    2,    2/
C     -----------
C     BEGIN CODE
C     -----------
     ...
     $   //' maximum ',NLOOPAMPS
      ENDIF
      ML5SOINDEX_FOR_LOOP_AMP = LOOPAMPORDERS(AMPID)

      END


      INTEGER FUNCTION ML5SQSOINDEX(ORDERINDEXA, ORDERINDEXB)
C     
...
C     FUNCTION
C     
      INTEGER ML5SOINDEX_FOR_SQUARED_ORDERS
C     
C     BEGIN CODE
C     
      DO I=1,NSO
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5GET_SQUARED_ORDERS_FOR_SOINDEX(SOINDEX,ORDERS)
C     
...
C      Not directly useful, but provided nonetheless.
      SUBROUTINE ML5GET_ORDERS_FOR_AMPSOINDEX(SOINDEX,ORDERS)
C     
...
C      completeness
      INTEGER FUNCTION ML5SOINDEX_FOR_AMPORDERS(ORDERS)
C     
...
C     
      DO I=1,NAMPSO
        ...
        ENDDO
        ML5SOINDEX_FOR_AMPORDERS = I
        RETURN
 1009   CONTINUE
      ENDDO

      WRITE(*,*) 'ERROR:: Stopping function ML5SOINDEX_FOR_AMPORDERS'
      WRITE(*,*) 'Could not find squared orders ',(ORDERS(I),I=1,NSO)
      STOP
...
C     
      ...
      SUBROUTINE SET_AUTOMATIC_CACHE_CLEARING(ONOFF)
C     
...
C     ----------
      ...
      SUBROUTINE SET_LEG_POLARIZATION(LEG_ID, LEG_POLARIZATION)
      IMPLICIT NONE
C     
C     ARGUMENTS
C     
...
C     ----------
      ...
      ELSE
        ...
        ENDDO
      ENDIF
      CALL SET_LEG_POLARIZATIONS(LEG_ID,LEG_POLARIZATIONS)

      END

      SUBROUTINE SET_LEG_POLARIZATIONS(LEG_ID, LEG_POLARIZATIONS)
      IMPLICIT NONE
C     
C     CONSTANTS
C     
...
C     leg is POLARIZATIONS(legID,1: POLARIZATIONS(legID,0)   ).
      ...
      COMMON/BORN_BEAM_POL/BORN_POLARIZATIONS
