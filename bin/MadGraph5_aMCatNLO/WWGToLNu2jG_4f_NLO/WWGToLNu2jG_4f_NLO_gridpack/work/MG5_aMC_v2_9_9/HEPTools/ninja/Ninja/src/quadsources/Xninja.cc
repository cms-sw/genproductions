// Definition of the method quadninja::Amplitude::higherRankEvaluate().
// This method is private and is called by
// quadninja::Amplitude::evaluate() if rank == n_props+1, provided that
...
#include <quadsources/integermath.hh>
#include <quadsources/cuts_utils.hh>
#include <quadsources/coefficient_level_subs.hh>
#include <quadsources/integral_library_wrapper.hh>
#include <quadsources/s_mat_wrapper.hh>

using namespace quadninja;
using namespace x1cuts;
using namespace std;
using namespace quadninja_integer_math;
  ...
  int Amplitude<MassType>::higherRankEvaluate(Numerator & num)
  {
    ...
    if (Options::verb) {
      ...
      if (return_val == Amplitude::SUCCESS) {
        (*Options::out) << "quadninja::Amplitude is returning SUCCESS" << endl;
      } else {
        (*Options::out) << "quadninja::Amplitude is returning TEST_FAILED" << endl;
      }
      (*Options::out) << "\n\n----------------------------"
                      << "----------------------------" << endl;
    }
...
#define QUADNINJA_IMPLEMENTING_X1RANK    // implement extra-rank here
//#include <coefficient_level_subs.cxx>
#include <quadsources/ninja_implem.cxx>   // the implementation is in cuts_implem.cxx
#undef QUADNINJA_IMPLEMENTING_X1RANK  // extra-rank implementation ends here
