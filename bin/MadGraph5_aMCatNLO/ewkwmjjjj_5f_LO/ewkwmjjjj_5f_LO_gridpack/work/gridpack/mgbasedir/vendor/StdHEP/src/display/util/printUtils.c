/*******************************************************************************
*									       *
* printUtils.c -- Fermilab Nirvana library Printer Menu	& Printing Routines    *
*									       *
...
*									       *
*******************************************************************************/
static char SCCSID[] = "@(#)printUtils.c	1.19	9/26/94";
...
static void printButtonCB(Widget widget, caddr_t client_data, caddr_t call_data)
{
    ...
    if (spawn_sts != SS$_NORMAL) {
       	DialogF(DF_WARN, widget, 1,
       		"Unable to Print:\n%d - %s\n  spawnFlags = %d\n",
       		"Acknowledged", spawn_sts, strerror(EVMSERR, spawn_sts),
       		spawnFlags);
        return;
    }
#else
...
static void getVmsQueueDefault(char *defqueue)
{
    ...
    if (translate_sts != SS$_NORMAL && translate_sts != SS$_NOLOGNAM){
       ...
       DialogF(DF_WARN,Label2,1,"Error translating SYS$PRINT","Acknowledged");
       defqueue[0] = '\0';
    }
    else
