// Definition of the public method quadninja::Amplitude::evaluate() and
...
namespace quadninja {
  ...
  int Amplitude<MassType>::evaluate(Numerator & num)
  {
    ...
    if (rank>n+1) {
      ...
      QUADNINJA_THROW (invalid_argument("Rank is too high."));
    } else if (rank==n+1) {
      return higherRankEvaluate(num);
    }
#else // QUADNINJA_X1RANK
    if (rank>n) {
      cerr << "ERROR IN NINJA: "
...
#include <quadsources/ninja_implem.cxx>   // the implementation is in ninja_implem.cxx
