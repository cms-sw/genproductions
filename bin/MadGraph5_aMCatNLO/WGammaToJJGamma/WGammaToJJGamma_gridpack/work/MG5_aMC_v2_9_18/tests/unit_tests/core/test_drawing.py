...
class TestFeynmanLine(unittest.TestCase):
    ...
    def  test_def_begin_end_point(self):
        ...
        self.assertIs(self.my_line.begin, self.my_vertex)
        self.my_line.def_begin_point(self.my_vertex2)
        self.assertIs(self.my_line.begin, self.my_vertex2)

        #test end point
        self.my_line.def_end_point(self.my_vertex)
        self.assertIs(self.my_line.end, self.my_vertex)
        self.assertIs(self.my_line.end, self.my_vertex)

        #test if the vertex references the line correctly
        self.assertIn(self.my_line, self.my_vertex.lines)
        self.assertIn(self.my_line, self.my_vertex2.lines)

        #check that the swithching method runs fine.
        self.my_line.inverse_begin_end()
        self.assertIs(self.my_line.begin, self.my_vertex)
        self.assertIs(self.my_line.end, self.my_vertex2)
...
class TestVertexPoint(unittest.TestCase):
    ...
    def test_building(self):
        ...
        self.assertIsInstance(my_vertex, drawing.VertexPoint)
        ...
        self.assertIsNot(my_vertex, self.vertex)
        my_vertex.value = 2
        ...
        self.assertNotIn('value', list(self.vertex.keys()))
        ...
        self.assertIsNot(my_vertex2, self.vertex)
        self.assertIsNot(my_vertex2, my_vertex)

        # Check that adding a line acts only on one vertex.
        my_vertex.add_line(self.line1)
        self.assertNotIn(self.line1, my_vertex2.lines)
    ...
    def test_redef_position(self):
        ...
        self.assertAlmostEqual(self.line1.has_ordinate(0.2), 0.45)
    ...
    def test_add_line(self):
        ...
        self.assertIn(self.line1, my_vertex.lines)
        my_vertex.add_line(self.line1)
    ...
    def test_remove_line(self):
        ...
        self.assertNotIn(self.line1, my_vertex.lines)
    ...
    def test_fuse_vertex(self):
        ...
        self.assertIs(vertex1, line3.begin)
        self.assertIs(vertex1, line4.begin)
        self.assertIs(vertex1, line1.end)
        self.assertIs(vertex1, line2.end)
...
class TestFeynmanDiagram(unittest.TestCase):
    ...
    def test_load_diagram(self):
        ...
        for obj in self.mix_drawing.vertexList:
            self.assertIsInstance(obj, drawing.VertexPoint)
        for obj in self.mix_drawing.lineList:
            self.assertIsInstance(obj, drawing.FeynmanLine)
    ...
    def test_line_ordering_in_load(self):
        ...
        for i, line in enumerate(self.mix_drawing.lineList):
            self.assertEqual(
                self.vertex_identification(line.begin),
                begin_tag[i]
            )
            self.assertEqual(
                self.vertex_identification(line.end),
                end_tag[i]
            )
    ...
    def test_define_level(self):
        ...
        for i in range(0, 10):
            self.assertEqual(
                self.mix_drawing.vertexList[i].level,
                level_solution[i]
            )
            self.assertEqual(
                len(self.mix_drawing.vertexList[i].lines),
                number_of_line[i]
            )
        ...
        for i in range(0, 6):
            self.assertEqual(
                self.s_drawing.vertexList[i].level,
                level_solution[i]
            )
        ...
        for i in range(0, 6):
            self.assertEqual(
                self.t_drawing.vertexList[i].level,
                level_solution[i]
            )
    ...
    def test_find_t_channel_vertex(self):
        ...
        for vertex in t_vertex:
            ...
            self.assertNotIn(vertex, level0)
    ...
    def test_find_initial_vertex_position(self):
        ...
        for i in range(0, 10):
            ...
            self.assertAlmostEqual(
                self.mix_drawing.vertexList[i].pos_x,
                x_position[i]
            )
            self.assertAlmostEqual(
    ...
    def test_fermion_flow(self):
        ...
        for line in t_lines:
            if line.is_fermion():
                self.assertGreater(line.begin.pos_y, line.end.pos_y)
        ...
        for line in t_lines:
            if line.is_fermion():
                self.assertLess(line.begin.pos_y, line.end.pos_y)
    ...
    def test_diagram_equality(self):
        ...
        for i in range(8):
            for j in range(i, 8):
                if i == j:
                    self.assertEqual(diagram_list[i], diagram_list[j])
                else:
                    self.assertNotEqual(diagram_list[i], diagram_list[j])
    ...
    def test_horizontal_mode(self):
        ...
        for vertex in vertex_list:
            self.assertNotEqual(vertex.pos_x, 0)
