...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ d~ > e+ e- d~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ d~ > mu+ mu- d~ d~
      PD(IPROC)=DX1*DX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > e+ e- s~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ s~ > mu+ mu- s~ s~
      PD(IPROC)=SX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b~ b~ > e+ e- b~ b~
      PD(IPROC)=BX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! b~ b~ > mu+ mu- b~ b~
      PD(IPROC)=BX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG6 = PD(0)
