...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d~ s~ > e+ e- d~ s~
      PD(IPROC)=DX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ s~ > mu+ mu- d~ s~
      PD(IPROC)=DX1*SX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ b~ > e+ e- d~ b~
      PD(IPROC)=DX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! d~ b~ > mu+ mu- d~ b~
      PD(IPROC)=DX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ b~ > e+ e- s~ b~
      PD(IPROC)=SX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! s~ b~ > mu+ mu- s~ b~
      PD(IPROC)=SX1*BX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG44 = PD(0)
