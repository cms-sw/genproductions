...
static void destroy(SpinWidget w)
{
    ...
    XtRemoveAllCallbacks (w, XmNresizeCallback, w->spin.resize);
    XtRemoveAllCallbacks (w, XmNbtn2Callback, w->spin.btn2);
    XtRemoveAllCallbacks (w, XmNbtn3Callback, w->spin.btn3);
    XtRemoveAllCallbacks (w, XmNredisplayCallback, w->spin.redisplay);
}
...
static void btn2AP(SpinWidget w, XEvent *event, char *args, int n_args)
{
    ...
    XtCallCallbacks (w, XmNbtn2Callback, &cbStruct);
}
...
static void btn3AP(SpinWidget w, XEvent *event, char *args, int n_args)
{
    ...
    cbStruct.reason = XmCR_INPUT;
    cbStruct.event = event;
    XtCallCallbacks (w, XmNbtn3Callback, &cbStruct);
}
...
void SpinSetSegments(Widget w, SpinSegment *segments, int nSegments)
{
    ...
    if (sw->spin.nSegments != 0) {
    	XtFree(sw->spin.segments);
    	sw->spin.nSegments = 0;
    }
    if (sw->spin.nPoints != 0) {
    	XtFree(sw->spin.points);
    	sw->spin.nPoints = 0;
    }
    if (nSegments != 0) {
    	sw->spin.segments =
    		(SpinSegment *)XtMalloc(sizeof(SpinSegment) * nSegments);
    	bcopy(segments, sw->spin.segments, sizeof(SpinSegment) * nSegments);
...
void SpinSetPoints(Widget w, SpinPoint *points, int nPoints)
{
    ...
    if (nPoints != 0) {
    	...
    	bcopy(points, sw->spin.points, sizeof(SpinPoint) * nPoints);
...
{        
    ...
    if (XtIsRealized(w))
    	XtCallCallbacks(w, XmNredisplayCallback, NULL);
}
...
static void rotateProc(SpinWidget w, XtIntervalId *id)
{
    ...
    w->spin.timeOutID = XtAppAddTimeOut(XtWidgetToApplicationContext(w),
    					MIN_REFRESH_TIME, rotateProc, w);
}
