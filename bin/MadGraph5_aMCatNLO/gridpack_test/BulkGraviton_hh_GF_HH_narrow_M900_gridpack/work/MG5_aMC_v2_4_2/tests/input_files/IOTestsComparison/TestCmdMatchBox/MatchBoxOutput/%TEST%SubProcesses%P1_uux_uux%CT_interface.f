...
C     CALL CTSINIT(THRS,LOOPLIB)
      ...
      SUBROUTINE MG5_1_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L
     $ ,S_MAT)
C     
...
C            component
     ...
     $          + ABS(P_LOOP(J,K))
      ...
      SUBROUTINE MG5_1_MP_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_LOOP,M2L
     $ ,S_MAT)
C     
C     Helper function that compute the loop kinematic matrix with
C      proper thresholds
...
C     ===========================================
      ...
      SUBROUTINE MG5_1_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
C     Module used
C     
      USE MNINJA
...
C      the loop is read for CTMode equal to 2 or 4.
      CALL MG5_1_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN_DUMMY,M2L)
...
C     coefficients must be changed accordingly
      CALL MG5_1_INVERT_MOMENTA_IN_POLYNOMIAL(CURR_MAXCOEF,TENSORCOEFS)
...
C     Compute the kinematic matrix
      DO J=1,NLOOPLINE
        ...
        ENDDO
      ENDDO
      CALL MG5_1_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_S_MAT,M2L,S_MAT)
...
C     
      SUBROUTINE MG5_1_MP_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,RES,STABLE)
C     
C     Module used
C     
      USE MNINJA
...
C     For the direction test, we must switch the direction in which
C      the loop is read for CTMode equal to 2 or 4.
      CALL MG5_1_MP_SWITCH_ORDER(CTMODE,NLOOPLINE,PL,PDEN_DUMMY,MP_M2L)
...
C     The loop momentum is in fact q_loop -> -q_loop, so that the
C     coefficients must be changed accordingly
      CALL MP_MG5_1_INVERT_MOMENTA_IN_POLYNOMIAL(CURR_MAXCOEF
     $ ,MP_TENSORCOEFS)
...
C     Compute the kinematic matrix
      DO J=1,NLOOPLINE
        ...
        ENDDO
      ENDDO
      CALL MG5_1_MP_BUILD_KINEMATIC_MATRIX(NLOOPLINE,P_S_MAT,MP_M2L
     $ ,MP_S_MAT)
...
C         Ninja is used
          IF (.NOT.DOING_QP) THEN
            CALL MG5_1_NINJA_LOOP(NLOOPLINE,PL,M2L,RANK,LOOPRES(1
     $       ,SQUAREDSOINDEX,LOOPNUM),S(SQUAREDSOINDEX,LOOPNUM))
          ELSE
            CALL MG5_1_MP_NINJA_LOOP(NLOOPLINE,MP_PL,M2L,RANK
