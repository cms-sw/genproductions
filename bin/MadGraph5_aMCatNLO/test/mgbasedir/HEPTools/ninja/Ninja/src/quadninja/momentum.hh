...
#ifndef QUADNINJA_MOMENTUM_HH
#define QUADNINJA_MOMENTUM_HH
...
#if 1
# define QUADNINJA_MOM_EXPR(mu,expr) do { \
    unsigned mu = 0;  expr;              \
    mu = 1;  expr;                      \
    mu = 2;  expr;                      \
    mu = 3;  expr;                      \
...
namespace quadninja {
  ...
  private:
    quadninja::details::Array4D<Real> data;
  ...
  private:
    quadninja::details::Array4D<Complex> data;
  ...
  {
    return RealMomentum(quadninja::real(p[0]), quadninja::real(p[1]),
                        quadninja::real(p[2]), quadninja::real(p[3]));
  }
  
  // Immaginary part of a complex momentum
  inline RealMomentum imag(const ComplexMomentum & p)
  {
    return RealMomentum(quadninja::imag(p[0]), quadninja::imag(p[1]),
                        quadninja::imag(p[2]), quadninja::imag(p[3]));
  }
  
  // Complex conjugate of a complex momentum
  inline ComplexMomentum conj(const ComplexMomentum & p)
  {
    return ComplexMomentum(quadninja::conj(p[0]), quadninja::conj(p[1]),
                           quadninja::conj(p[2]), quadninja::conj(p[3]));
  }

} // namespace quadninja

#endif // QUADNINJA_MOMENTUM_HH
