...
C     
C     Process: u d~ > wp+ > e+ ve WEIGHTED<=6 @1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     ----------
     ...
     $   *SMALL_WIDTH_TREATMENT)), ZERO)
        IF(ZERO.NE.0D0) FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MD
     $   *SMALL_WIDTH_TREATMENT)), ZERO)
        IF(ZERO.NE.0D0) FK_ZERO = SIGN(MAX(ABS(ZERO), ABS(MDL_MU
     $   *SMALL_WIDTH_TREATMENT)), ZERO)
        IF(MDL_WWP.NE.0D0) FK_MDL_WWP = SIGN(MAX(ABS(MDL_WWP),
     $    ABS(MDL_MWP*SMALL_WIDTH_TREATMENT)), MDL_WWP)
     ...
     $   *SMALL_WIDTH_TREATMENT)), ZERO)
      ...
      CALL FFV2_3_3(W(1,1),W(1,2),GC_83,GC_101,MDL_MWP, FK_MDL_WWP,W(1
     $ ,5))
C     Amplitude(s) for diagram number 1
      CALL FFV2_3_0(W(1,3),W(1,4),W(1,5),GC_74,GC_92,AMP(1))
