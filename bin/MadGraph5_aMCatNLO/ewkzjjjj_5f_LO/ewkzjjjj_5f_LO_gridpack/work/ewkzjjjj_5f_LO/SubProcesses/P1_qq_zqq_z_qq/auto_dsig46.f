      DOUBLE PRECISION FUNCTION DSIG46(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,46)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG46=DSIGN(PD(0)*CONV*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C     Generate events only if IMODE is 0.
      IF(IMODE.EQ.0.AND.DABS(DSIG46).GT.0D0)THEN
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG46*WGT,46)
      ENDIF
