      DOUBLE PRECISION FUNCTION DSIG29(PP,WGT,IMODE)
C     ****************************************************
C     
...
C     Continue only if IMODE is 0, 4 or 5
     ...
     $   ))
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! u u~ > d d~ s s~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u~ > d d~ b b~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > d d~ d d~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > d d~ b b~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u~ > b b~ s s~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! u u~ > b b~ b b~
      PD(IPROC)=U1*UX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > b b~ d d~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! c c~ > b b~ b b~
      PD(IPROC)=C1*CX2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG29 = PD(0)
...
C     Apply the bias weight specified in the run card (default is 1.0)
      DSIGUU=DSIGUU*CUSTOM_BIAS(PP,DSIGUU,29)

      DSIGUU=DSIGUU*NFACT

      IF (DSIGUU.LT.1D199) THEN
C       Set sign of dsig based on sign of PDF and matrix element
        DSIG29=DSIGN(PD(0)*CONV*DSIGUU,DSIGUU*PD(IPSEL))
      ELSE
...
C     Generate events only if IMODE is 0.
      IF(IMODE.EQ.0.AND.DABS(DSIG29).GT.0D0)THEN
C       Call UNWGT to unweight and store events
        CALL UNWGT(PP,DSIG29*WGT,29)
      ENDIF
