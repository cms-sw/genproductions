...
C      if(incbsmdm) then 
!===== use amplitude form 
C      call gg_dm_monojet_amp_S_top(p,msq)
C      return
C      endif
...
!======               = v/dm_lam * (1/prop) in full 
      propsq=1d0
!----- allow for deviations in top-mediator interaction from Higgs
      gdm=g_dmx*g_dmq
      
!      write(6,*) g_dmq,g_dmx,gdm
!      pause
!------ store old width 
      running_width=.false.
...
!===== g_x is defined as a rescaling of the Higgs style yukawa inteaction
         hdecay=hdecay*propsq*g_dmx**2*xmass**2/vevsq
      endif

      
      gg=+avegg*g_dmq**2*ehsvm3_dm(s(1,2),s(1,5),s(2,5),s34)*hdecay
      qq=+aveqq*g_dmq**2*ehsvm4_dm(s(1,2),s(1,5),s(2,5),s34)*hdecay
      qg=-aveqg*g_dmq**2*ehsvm4_dm(s(1,5),s(1,2),s(2,5),s34)*hdecay
      gq=-aveqg*g_dmq**2*ehsvm4_dm(s(2,5),s(1,5),s(1,2),s34)*hdecay
